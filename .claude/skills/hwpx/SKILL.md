...
- 초과에는 세 갈래가 있다 — ① 축약 ② 셀 폭 확대 ③ 행 추가. **①을 기본으로 삼지 말 것.** 검사가 ②③만 경고하기 때문에 "검사를 통과시키는 가장 쉬운 길"이 ①이 되고, 그래서 내용이 깎인다.
- 날짜·이름·전화번호·금액처럼 **줄여도 뜻이 안 상하는 값**은 ①이 맞다. 반면 활동명·행사명처럼 정보가 담긴 값은 ①이 곧 훼손이다 — `센터 이동 및 로봇 스포츠 챌린지` → `이동·로봇 체험`이 2026. 8. 20. v3에서 검토 4점을 받은 실제 사례다(「실패 이력」 참조). 이런 값은 ②③으로 가고, 구조를 바꿔야 하면 **사용자에게 요청**한다
...
| **2026. 8. 20. (v2)** | **표 셀 글자가 셀 폭을 넘겨 렌더링에서 줄바꿈**(`불참 (` 뒤 닫는 괄호가 다음 줄로) | validate·page_guard·content_guard·gonmun_lint·COM 실열림·render_check **전부 PASS**. 예산 경고는 있었으나 `--allow-over-budget`으로 넘김 | **기계화됨(2026. 8. 20.)** — `--allow-over-budget`은 `--verified-by-render` 없이 거부되고, 묵인분은 `WARNING`으로 전부 출력된다. 품질 검사는 이 플래그로 더 이상 꺼지지 않는다 (`edit_hwpx.preflight_text_budget`, 회귀 테스트 5건) |
...
18. **셀 예산 초과는 값의 성격에 따라 처리**: 날짜·이름·금액은 축약, 활동명·행사명은 셀 폭 확대나 행 추가(구조 변경은 사용자에게 요청). `--allow-over-budget`은 `--verified-by-render`와 짝으로만 동작하며 렌더 이미지를 실제로 본 뒤에만 쓴다 (2026. 8. 20. 렌더링 붕괴)
